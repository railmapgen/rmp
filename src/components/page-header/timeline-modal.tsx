...
export default function TimelineModal({ isOpen, onClose }: TimelineModalProps) {
    ...
    React.useEffect(() => {
        ...
        if (pickState.step === 'pickingStart') {
            ...
            if (first && (id.startsWith('stn_') || id.startsWith('misc_node_'))) {
                ...
                if (path) {
                    // Calculate auto-reverse settings for each edge in the path
                    const newEntries = calculateAutoReverseForPath(graph.current, path);
                    // Merge new entries with existing timeline, preserving reverse properties
                    updateTimeline(deduplicateTimelineEntries(timeline, newEntries));
                } else {
    ...
    const handleSetItemVersion = (index: number, version: number | undefined) => {
        updateTimeline(
            timeline.map((item, itemIndex) => {
                if (itemIndex !== index || !isNodeId(item.id)) return item;
                if (version === undefined || version <= 1) {
                    const { version: _version, ...rest } = item;
                    return rest;
                }
                return { ...item, version };
            }),
                                    ...
                                    <VStack align="stretch" spacing={1}>
                                        {timeline.map((entry, index) => (
                                            <TimelineItem
                                                ...
                                                versionOptions={getNodeVersionOptions(entry.id as NodeId)}
                                                onSelectionChange={checked => handleToggleLineSelection(index, checked)}
                                                onRemove={handleRemoveItem}
                                                onToggleReverse={handleToggleItemReverse}
                                                onSetVersion={version => handleSetItemVersion(index, version)}
...
function TimelineItem({
    ...
    onSetVersion: (version: number | undefined) => void;
    onDragStart: () => void;
                ...
                {showVersionSelector && (
                    <Select
                        ...
                        onChange={e => {
                            ...
                            onSetVersion(value <= 1 ? undefined : value);
                        }}
                    >
                        {versionOptions.map(v => (
                            <option key={v} value={v}>
                                {getNodeVersionName(entry.id as NodeId, v)}
                            </option>
                        ))}
                    </Select>
                )}
